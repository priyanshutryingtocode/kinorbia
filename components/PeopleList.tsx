  ...
  return (
    <div className="space-y-3">
      {people.map((person) => {
        ...
        const href = isSelf ? "/profile" : person.username ? `/u/${person.username}` : null;
        const isFollowing = following.includes(person.email);

        const avatar = (
          <span className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full bg-neutral-800">
            {person.image ? (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              {href ? (
                ...
                <p className="block truncate font-bold text-white">{person.name}</p>
              )}
